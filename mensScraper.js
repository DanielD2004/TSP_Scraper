...
INSERT INTO tsp_mens (id, date, time, venueName, homeTeamName, homeScore, awayTeamName, awayScore)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
ON CONFLICT(id) DO NOTHING;
`

const retrieveGameIdsQuery = `
    SELECT * FROM tsp_mens WHERE id = $1;
`
...
async function sendMessage(game, change=0) {
    ...
    let response = await axios.post('https://ntfy.sh/Mens2024TSP_Games', message, {
        headers: {
            'Title': 'NEW Mens SCORE',
            'Tags': 'softball',
            'Content-Type': 'text/plain',
            'Priority': 'urgent',
            'Actions': 'view, Open Site, https://www.thornhillslopitch.com/Score/Standings/109'
        }
    });
}

async function getData() {
    const { data } = await axios.get('https://www.thornhillslopitch.com/Score/Standings/109')
